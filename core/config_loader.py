...
if __name__ == "__main__":
    # test video generatore configuration
    config = Config()
    
    logger.info("**** testing vidgen configuration ****")
    logger.info(f"video resolution: {config.get('video.resolution')}")
    logger.info(f"target segments: {config.get('content.target_segments')}")
    logger.info(f"voice provider: {config.get('voiceover.provider')}")
    
    logger.info("**** api keys status ****")
    api_status = config.validate_api_keys()
    for api, available in api_status.items():
        status = "available" if available else "missing"
        logger.info(f"{api.capitalize()}: {status}")
    
    logger.info("**** vidgen configuration test complete ****")
